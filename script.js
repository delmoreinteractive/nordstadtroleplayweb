...
const siteConfig = {
  ...
  dispatch: {
    ...
    text: "Das Roblox-Spiel ist aktuell in Entwicklung. Verfolge Fortschritte und Ankündigungen auf unserem Discord."
  },
...
/*
  ...
  - count: aktuell besetzte Personen
  - vacancies: gesuchte Anzahl; unterstützt auch Bereiche wie "2–4"
*/
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const renderManagement = () => {
    ...
    managementList.innerHTML = managementRoles.map((role, index) => {
      ...
      const vacancies = String(role.vacancies ?? "0");
      const holder = role.filled
        ? (role.holder || "Besetzt")
        : `${vacancies} ${vacancies === "1" ? "Position" : "Positionen"} gesucht`;
      const slotValue = role.filled ? String(Number(role.count) || 1) : vacancies;
      const slotLabel = role.filled ? "besetzt" : "gesucht";
      ...
      return `
        <article class="management-role reveal is-visible" data-filled="${role.filled}">
          ...
          <span class="management-role__status">
            ${role.filled ? "Besetzt" : "Gesucht"}
          </span>
          <span
            class="management-role__count"
            title="${escapeHtml(slotValue)} ${slotLabel}"
            aria-label="${escapeHtml(slotValue)} ${slotLabel}"
          >
            <strong>${escapeHtml(slotValue)}</strong>
    ...
    const filledCount = managementRoles.reduce(
      (sum, role) => sum + (role.filled ? (Number(role.count) || 1) : 0),
      0
    );

    const openMinimum = managementRoles.reduce((sum, role) => {
      if (role.filled) return sum;
      const values = String(role.vacancies).match(/\d+/g)?.map(Number) || [0];
      return sum + values[0];
    }, 0);
